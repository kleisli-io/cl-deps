...
let
  ...
  # Flatten transitive deps via memoized post-order DFS
  flatten = packages:
    let
      go = seen: pkgs: acc:
        builtins.foldl'
          (state: pkg:
            let id = identity pkg;
            in if builtins.elem id state.seen
               then state
               else
                 let
                   depsResult = go (state.seen ++ [ id ]) (pkg.lispDeps or []) state.acc;
                 in {
                   seen = depsResult.seen;
                   acc = depsResult.acc ++ [ pkg ];
                 })
          { inherit seen acc; }
          pkgs;
    in (go [] packages []).acc;
